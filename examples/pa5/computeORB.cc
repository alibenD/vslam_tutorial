    ...
    for (auto &kp : keypoints)
    {
      ...
      for(int u = -half_patch_size; u < half_patch_size; ++u)
      {
        for(int v= -half_patch_size; v < half_patch_size; ++v)
        {
          auto x_new = x+u;
          auto y_new = y+v;
          if(x_new > cols || y_new > rows || x_new < 0 || y_new < 0)
          {
            continue;
          }
          auto intensity = image.at<unsigned char>(y_new, x_new);
          m10 += u*intensity;
          m01 += v*intensity;
          //m00 += image.at<unsigned char>(x_new, y_new);
          //std::cout << "x: " << x_new
          //          << "\ty: " << y_new
          //          << "\tu: " << u
          //          << "\tv: " << v
      ...
      //          << "\tm00: " << m00
      //          << std::endl;
      //std::cout << "m01/m00: " << m01/(double)m00
      //          << "\tm10/m00: " << m10/(double)m00
      //          << "\tm00/m00: " << m00/(double)m00
      //          << std::endl;
      //std::cout << "x: " << x
      //          << "y: " << y
      //          << std::endl;
      kp.angle = atan2((double)m01, (double)m10)/M_PI*180; // compute kp.angle 
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc)
{
  ...
  for (auto &kp: keypoints)
  {
    ...
    auto theta = kp.angle/180*M_PI;
    for (int i = 0; i < 256; ++i)
    {
      // START YOUR CODE HERE (~7 lines)
      double xp = ORB_pattern[i*4 + 0];
      double yp = ORB_pattern[i*4 + 1];
      double xq = ORB_pattern[i*4 + 2];
      double yq = ORB_pattern[i*4 + 3];
      //auto xp = kp.pt.x + ORB_pattern[i*4 + 0];
      //auto yp = kp.pt.y + ORB_pattern[i*4 + 1];
      //auto xq = kp.pt.x + ORB_pattern[i*4 + 2];
      //auto yq = kp.pt.y + ORB_pattern[i*4 + 3];
      double xp_r = xp * cos(theta) - yp * sin(theta) + kp.pt.x;
      double yp_r = xp * sin(theta) + yp * cos(theta) + kp.pt.y;
      double xq_r = xq * cos(theta) - yq * sin(theta) + kp.pt.x;
      double yq_r = xq * sin(theta) + yq * cos(theta) + kp.pt.y;
      //std::cout << "xp: " << xp
      //          << "\typ: " << yp
      //          << "\txp_r: " << xp_r
      //          << "\typ_r: " << yp_r
      ...
      {
        d.clear();
        //std::cout << "Break descriptor" << std::endl;
        break;
      }
      if( image.at<unsigned char>(ceil(yp_r), ceil(xp_r)) >
          image.at<unsigned char>(ceil(yq_r), ceil(xq_r)))
      {
        d[i] = false;
      }
      else
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
  ...
  for(size_t index_d1 = 0; index_d1 < desc1.size(); ++index_d1)
  {
    int min_dist = 256;
    int min_index = index_d1;
    if(desc1[index_d1].size() != 256)
    {
      continue;
    }

    for(size_t index_d2 = 0; index_d2 < desc2.size(); ++index_d2)
    {
      int dist = 0;
      if(desc2[index_d2].size() != 256)
      {
        continue;
      }

      for(size_t index=0; index < desc1[index_d1].size(); ++index)
      {
        if(desc1[index_d1][index] != desc2[index_d2][index])
        {
          ++dist;
        }
      }
    ...
    if(min_dist < d_max)
    {
      cv::DMatch match(index_d1, min_index, min_dist);
      matches.push_back(match);
    }
  }
  //std::cout << "Size: matches: " << matches.size() << std::endl;
  // END YOUR CODE HERE
