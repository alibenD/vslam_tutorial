...
string file_1 = "../data/room1.png";  // first image
string file_2 = "../data/room2.png";  // second image
...
)
{
    ...
    for (int iter = 0; iter < iterations; iter++)
    {
      ...
      for (int x = -half_patch_size; x < half_patch_size; x++)
      {
        for (int y = -half_patch_size; y < half_patch_size; y++)
        {
          ...
          auto x_new = kp.pt.x + x;
          auto y_new = kp.pt.y + y;
          if(x_new > img1.cols || y_new > img1.rows)
          {
            continue;
          }
          //double error = double(img1.at<uchar>(kp.pt.y + y + dy, kp.pt.x + x + dx) -
          //                      img2.at<uchar>(kp.pt.y + y, kp.pt.x + x));
          double error = GetPixelValue(img2, x_new + dx, y_new + dy) -
                         GetPixelValue(img1, x_new, y_new);
          cost += error * error;
          Eigen::Vector2d J;  // Jacobian
          if (inverse == false)
          {
            // Forward Jacobian
            //double Ix = (img1.at<uchar>(kp.pt.y + y, kp.pt.x + x) -
            //            img1.at<uchar>(kp.pt.y + y, kp.pt.x)) / double(x);
            //double Iy = (img1.at<uchar>(kp.pt.y + y, kp.pt.x + x) -
            //            img1.at<uchar>(kp.pt.y, kp.pt.x + x)) / double(y);
            double Ix = (GetPixelValue(img2, kp.pt.x + x + dx + 1,
                                             kp.pt.y + y + dy)
                        -GetPixelValue(img2, kp.pt.x + x + dx - 1,
                                             kp.pt.y + y + dy)) / 2;
            double Iy = (GetPixelValue(img2, kp.pt.x + x + dx,
                                             kp.pt.y + y + dy + 1)
                        -GetPixelValue(img2, kp.pt.x + x + dx,
                                             kp.pt.y + y + dy - 1)) / 2;
            J(0,0) = Ix;
            J(1,0) = Iy;
            if(x == 0 || isnan(Ix))
      ...
      //std::cout << "delta x: " << update.transpose() << std::endl;
      // TODO END YOUR CODE HERE
      if (isnan(update[0]))
      {
  ...
  std::cout << "kp1: " << kp1.size() << "\tkp2: " << kp2.size() << std::endl;
}
        ...
        bool inverse)
{
    ...
    {
      std::vector<cv::KeyPoint> kp1_pyr;
      for(size_t i=0; i<kp1.size(); ++i)
      {
        cv::KeyPoint pt_pyr = kp1[i];
